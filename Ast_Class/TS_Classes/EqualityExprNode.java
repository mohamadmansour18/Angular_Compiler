...
public class EqualityExprNode extends Node {
    ...
    public void setOps(ArrayList<String> ops) { this.ops = ops; }
    ...
    public String generate(GenContext ctx) {
        // تنظيف بسيط لإزالة ';' النهائية إن وُجدت
        ...
        if (first.isEmpty()) first = "undefined";
        sb.append(first);

        for (int i = 0; i < ops.size(); i++) {
            String op = ops.get(i);
            // خرائط التوكنات إلى JS
            String jsOp =
            "TEQUAL".equals(op)      ? "===" :
            "EQUALEQUAL".equals(op)  ? "=="  :
            "NTEQUAL".equals(op)     ? "!==" :
            "NOTEQUAL".equals(op)    ? "!="  :
            op;
            ...
            if (rhs.isEmpty()) rhs = "undefined";

            sb.append(" ").append(jsOp).append(" ").append(rhs);
        }
